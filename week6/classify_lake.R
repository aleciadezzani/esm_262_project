...
classify_lake = function(nutrient, temperature, nutrient_threshold_med=5,
  ...
  while ( (numhotday < 5) && (i <= length(temperature))) {  #make sure to set a stop point so no infinite loop
      if (temperature[i] > temperature_threshold)
        # we have another day with greater than 10 so accumulate
        numhotday = numhotday+1  #if temp is greated than threshold, add one to numhotdays
      else
